...
/**
 * ADDED to keep track of unique ID on objects
 */
(function () {
...
Interpreter.prototype.pseudoToNative = function (
  ...
  if (this.isa(pseudoObj, this.REGEXP)) {
    ...
    if (add_unique) {
      nativeRegExp.__uniqueid = Object.id(pseudoObj);
    }
  ...
  if (this.isa(pseudoObj, this.DATE)) {
    ...
    if (add_unique) {
      nativeDate.__uniqueid = Object.id(pseudoObj);
    }
  ...
  if (
    ...
    if (add_unique) {
      nativeBox.__uniqueid = Object.id(pseudoObj);
    }
  ...
  // ADDED to support Maps and Sets using
  // https://github.com/anonyco/Javascript-Fast-Light-Map-WeakMap-Set-And-WeakSet-JS-Polyfill/blob/master/mapPolyfill.src.js

  if (
    this.globalObject.properties.Set != undefined &&
    this.isa(pseudoObj, this.globalObject.properties.Set)
  ) {
    const nativeObj = new Set();
    if (add_unique) {
      nativeObj.__uniqueid = Object.id(pseudoObj);
    }

    const pseudoVals = pseudoObj.properties.k.properties;
    for (let key in pseudoVals) {
      nativeObj.add(this.pseudoToNative(pseudoVals[key], cycles, add_unique));
    }
    cycles.native.push(nativeObj);
    return nativeObj;
  }
  ...
  if (
    this.globalObject.properties.Map != undefined &&
    this.isa(pseudoObj, this.globalObject.properties.Map)
  ) {
    const nativeObj = new Map();
    if (add_unique) {
      nativeObj.__uniqueid = Object.id(pseudoObj);
    }

    const pseudoKeys = pseudoObj.properties.k.properties;
    const pseudoVals = pseudoObj.properties.v.properties;

    for (let arrayKey in pseudoKeys) {
      const key = pseudoKeys[arrayKey];
      const val = this.pseudoToNative(pseudoVals[arrayKey], cycles, add_unique);
      nativeObj.set(key, val);
    }
  ...
  // END Map, Set support changes

  // Add support for function descriptions.
  // Note - DO NOT MAP FUNCTION BODY FOR SECURITY REASONS

  if (this.isa(pseudoObj, this.FUNCTION)) {
    const params = pseudoObj.node
      ? pseudoObj.node?.params.map((node) => node.name)
      : [];

    const nativeObj = new Function(
      ...params,
      `// console.log("sandbox functions must not be executed outside")`
    );

    nativeObj.functionName = pseudoObj.node?.id?.name || "Function";
    nativeObj.bodyLoc = pseudoObj.node.loc;
    nativeObj.params = params;
    if (add_unique) {
  ...
  // END function descripion changes
  ...
  for (var key in pseudoObj.properties) {
    val = this.pseudoToNative(pseudoObj.properties[key], cycles, add_unique);
